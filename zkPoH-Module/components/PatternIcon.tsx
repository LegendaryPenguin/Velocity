  ...
  switch (type) {
    case "circle":
      return (
        <svg viewBox={`0 0 ${s} ${s}`} className="h-full w-full drop-shadow-sm">
          <defs>
            <radialGradient id="zkpoh-cg" cx="40%" cy="35%">
              <stop offset="0%" stopColor="#c4b5fd" />
              <stop offset="100%" stopColor="#7c3aed" />
            </radialGradient>
          </defs>
          <circle cx={h} cy={h} r={16} fill="url(#zkpoh-cg)" />
    ...
    case "triangle":
      return (
        <svg viewBox={`0 0 ${s} ${s}`} className="h-full w-full drop-shadow-sm">
          <defs>
            <linearGradient id="zkpoh-tg" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#f0abfc" />
              <stop offset="100%" stopColor="#a855f7" />
            </linearGradient>
          </defs>
          <polygon points="24,6 42,42 6,42" fill="url(#zkpoh-tg)" />
        </svg>
      );
    case "plus":
      return (
        <svg viewBox={`0 0 ${s} ${s}`} className="h-full w-full drop-shadow-sm">
          <defs>
            <linearGradient id="zkpoh-pg" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="#67e8f9" />
              <stop offset="100%" stopColor="#06b6d4" />
            </linearGradient>
          </defs>
          <rect x={18} y={6} width={12} height={36} rx={3} fill="url(#zkpoh-pg)" />
          <rect x={6} y={18} width={36} height={12} rx={3} fill="url(#zkpoh-pg)" />
        </svg>
      );
    case "waves":
      return (
    ...
    case "star":
      return (
        <svg viewBox={`0 0 ${s} ${s}`} className="h-full w-full drop-shadow-sm">
          <defs>
            <linearGradient id="zkpoh-sg" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#fbbf24" />
              <stop offset="100%" stopColor="#f59e0b" />
            </linearGradient>
          </defs>
          <polygon points="24,4 29,18 44,18 32,27 36,42 24,33 12,42 16,27 4,18 19,18" fill="url(#zkpoh-sg)" />
        </svg>
      );
  }
}
