...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onVerifiedHuman: (payload: ExportedJson, score: number, options?: { simulatedBot?: boolean }) => void;
};
...
export function CaptchaModal({ isOpen, onClose, onVerifiedHuman }: Props) {
  ...
  const handleVerify = useCallback(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (simulateBot) {
        setModelLabel("BOT");
        onVerifiedHuman(toExportJson(model.score), model.score, { simulatedBot: true });
      } else {
        setModelLabel("HUMAN");
