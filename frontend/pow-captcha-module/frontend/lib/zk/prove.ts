...
export async function generateProof(
  ...
  console.log("[ZK prove] generateProof called, score:", inputScore, "urls:", { wasmUrl, zkeyUrl });
  ...
  if (score < SCORE_MIN || score > SCORE_MAX) {
    console.warn("[ZK prove] Score out of range:", score, "valid:", SCORE_MIN, "-", SCORE_MAX);
    return {
      success: false,
  ...
  try {
    console.log("[ZK prove] Calling snarkjs.groth16.fullProve...");
    const { proof, publicSignals } = await snarkjs.groth16.fullProve(
      { score },
      wasmUrl,
      zkeyUrl,
    );
    console.log("[ZK prove] fullProve succeeded, publicSignals:", publicSignals);
    return { success: true, proof, publicSignals };
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[ZK prove] fullProve failed:", err);
...
export async function submitProofForValidation(
  ...
  console.log("[ZK prove] submitProofForValidation, url:", url);
  ...
  try {
    ...
    console.log("[ZK prove] Validate API response:", res.status, data);
    if (!res.ok) {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[ZK prove] Validate fetch failed:", err);
