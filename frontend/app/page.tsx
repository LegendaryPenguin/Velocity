...
function ProductCard({ product }: { product: Product }) {
  return (
    <div className="group flex min-w-[180px] flex-shrink-0 flex-col rounded-lg border border-gray-200 bg-white p-3 shadow-sm transition hover:shadow-md">
      <div className={`relative mb-3 flex h-36 items-center justify-center rounded-md ${product.color}`}>
      ...
      <h3 className="mb-1 line-clamp-2 text-sm font-medium leading-tight text-gray-900 group-hover:text-orange-700">
        {product.name}
      </h3>
...
function Carousel({ title, products, accent }: { title: string; products: Product[]; accent?: string }) {
  ...
  return (
    <section className="mb-6">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button type="button" className="text-sm font-medium text-cyan-700 hover:text-orange-700 hover:underline">See all deals</button>
      </div>
...
function GridSection({ title, products }: { title: string; products: Product[] }) {
  return (
    <section className="mb-6 rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3">
        {products.slice(0, 4).map((p) => (
          <div key={p.id} className="group cursor-pointer">
            <div className={`mb-2 flex h-28 items-center justify-center rounded-md ${p.color}`}>
      ...
      <button type="button" className="mt-3 text-sm font-medium text-cyan-700 hover:text-orange-700 hover:underline">See more</button>
    </section>
  );
}
    ...
    <div className="min-h-screen bg-gray-100 font-sans text-gray-900">
      ...
      <header className="sticky top-0 z-50 bg-gray-900 text-white">
        <div className="mx-auto flex h-[60px] max-w-[1500px] items-center gap-4 px-4">
          ...
          <div className="flex flex-1 items-center">
            ...
            <input type="text" placeholder="Search VeloCart" className="h-[38px] flex-1 border-0 px-3 text-sm text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-400 lg:rounded-none" />
            <button type="button" className="flex h-[38px] w-11 items-center justify-center rounded-r-md bg-orange-400 hover:bg-orange-500">
          ...
          <Link href="/captcha" className="hidden flex-col text-xs leading-tight hover:text-orange-300 md:flex">
            <span className="text-gray-400">Hello, Sign in</span>
            <span className="font-bold">Account &amp; Lists</span>
          </Link>
          <button type="button" className="hidden flex-col text-xs leading-tight hover:text-orange-300 md:flex">
            <span className="text-gray-400">Returns</span>
            <span className="font-bold">&amp; Orders</span>
          </button>
          <button type="button" className="relative flex items-center gap-1 text-sm font-bold hover:text-orange-300">
            ...
            <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-orange-500 text-xs font-bold text-white">0</span>
            Cart
          </button>
        </div>
        <nav className="border-t border-gray-700 bg-gray-800">
          <div className="mx-auto flex max-w-[1500px] items-center gap-1 overflow-x-auto px-4 py-1 text-sm">
            <button type="button" className="flex items-center gap-1 whitespace-nowrap rounded px-2 py-1 font-bold hover:bg-gray-700">
            ...
            {CATEGORIES.slice(1).map((cat) => (
              <button key={cat} type="button" className="whitespace-nowrap rounded px-2 py-1 hover:bg-gray-700">{cat}</button>
            ))}
          </div>
        </nav>
      </header>
      ...
      <div className="relative">
        <div className={`flex h-[320px] items-center bg-gradient-to-r ${slide.bg} px-8 transition-all duration-500 md:h-[380px] md:px-16`}>
          <div className="mx-auto w-full max-w-[1500px]">
            ...
            <button type="button" className="mt-4 rounded-full bg-white px-6 py-2 text-sm font-bold text-gray-900 shadow hover:bg-gray-100">Shop now</button>
          </div>
        </div>
      ...
      <main className="mx-auto -mt-10 max-w-[1500px] px-4 pb-12">
        <div className="mb-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <GridSection title="Pick up where you left off" products={RECENTLY_VIEWED} />
          <GridSection title="Buy again" products={BUY_AGAIN} />
          <GridSection title="Top deals for you" products={DEALS} />
          <GridSection title="Recommended" products={RECOMMENDED} />
        </div>

        <Carousel title="Deals of the Day" products={DEALS} accent="text-red-700" />
        <Carousel title="Recommended for You" products={RECOMMENDED} />
        <Carousel title="Recently Viewed" products={RECENTLY_VIEWED} />
        ...
        <div className="my-8 rounded-lg border border-gray-200 bg-white p-6 text-center shadow-sm">
          ...
          <Link href="/captcha" className="inline-block rounded-full bg-yellow-400 px-8 py-2.5 text-sm font-bold text-gray-900 shadow hover:bg-yellow-500">Sign in</Link>
          <p className="mt-2 text-xs text-gray-500">New customer?{" "}<Link href="/captcha" className="text-cyan-700 hover:text-orange-700 hover:underline">Start here.</Link></p>
        </div>

        <Carousel title="Best Sellers in Electronics" products={makeProducts(20, 8)} />
        <Carousel title="Trending in Home & Kitchen" products={makeProducts(12, 8)} />
      </main>
      ...
      <footer className="bg-gray-800 text-gray-300">
        ...
        <div className="mx-auto grid max-w-[1500px] gap-8 px-6 py-10 text-sm sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <ul className="space-y-2"><li><button type="button" className="hover:underline">Careers</button></li><li><button type="button" className="hover:underline">About VeloCart</button></li><li><button type="button" className="hover:underline">Investor Relations</button></li></ul>
          </div>
          <div>
            <h4 className="mb-3 font-bold text-white">Make Money with Us</h4>
            <ul className="space-y-2"><li><button type="button" className="hover:underline">Sell products</button></li><li><button type="button" className="hover:underline">Become an Affiliate</button></li><li><button type="button" className="hover:underline">Advertise Your Products</button></li></ul>
          </div>
          <div>
            <h4 className="mb-3 font-bold text-white">Payment Products</h4>
            <ul className="space-y-2"><li><button type="button" className="hover:underline">VeloCart Rewards Card</button></li><li><button type="button" className="hover:underline">Shop with Points</button></li><li><button type="button" className="hover:underline">Reload Your Balance</button></li></ul>
          </div>
          <div>
            <h4 className="mb-3 font-bold text-white">Let Us Help You</h4>
            <ul className="space-y-2"><li><button type="button" className="hover:underline">Your Account</button></li><li><button type="button" className="hover:underline">Your Orders</button></li><li><button type="button" className="hover:underline">Returns &amp; Replacements</button></li><li><button type="button" className="hover:underline">Help</button></li></ul>
          </div>
        </div>
        <div className="border-t border-gray-700 py-6 text-center text-xs text-gray-500">
          ...
          <p className="mt-1">&copy; 2026 VeloCart Demo. For demonstration purposes only.</p>
        </div>
      </footer>
    </div>
  );
