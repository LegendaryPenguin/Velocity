...
spinner_run "Downloading modules" git clone --depth 1 https://github.com/Vegito2367/0g-modules.git
  ...
  if $INCLUDE_ZKPOH; then
    section "Integrating zkPoH module into Next.js app"

    mkdir -p "$APP_NAME/app/api/" && mv 0g-modules/zkPOH/api/* $APP_NAME/app/api/
    mkdir -p "$APP_NAME/components" && mv 0g-modules/zkPOH/components/* "$APP_NAME/components/"
    mv 0g-modules/zkPOH/frontend/* "$APP_NAME/app/"
    mkdir -p "$APP_NAME/lib/zk/" && mv 0g-modules/zkPOH/public/lib/* "$APP_NAME/lib/zk/"
    mkdir -p "$APP_NAME/public/zk/ " && mv 0g-modules/zkPOH/public/zk/* "$APP_NAME/public/zk/"
    mkdir -p "$APP_NAME/hooks/" && mv 0g-modules/zkPOH/hooks/* "$APP_NAME/hooks/"

    info_msg "zkPoH module files moved to project."
    info_msg "You can now import zkPoH components from '@/components' and API routes are set up under /api/validate."
    info_msg "There is a demo page at route /captcha."

    success_msg "zkPoH module integrated into Next.js app."
  fi
  ...
  npm add @0glabs/0g-serving-broker ethers snarkjs @tensorflow/tfjs @tensorflow-models/handpose
fi
